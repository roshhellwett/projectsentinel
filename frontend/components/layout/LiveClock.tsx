...
export function LiveClock({ variant = 'navbar', className = '' }: LiveClockProps) {
  ...
  if (!now) {
    return (
      <div aria-hidden="true" className={`h-6 w-32 bg-paper-2 border border-rule ${className}`} />
    );
  }
  ...
  return (
    ...
    >
      <span>{date}</span>
      <time dateTime={isoLabel} className="font-mono text-xs text-ink tabular-nums">{time}</time>
    </div>
  );
}
