...
export const HeroCard = memo(function HeroCard({ post, badge = 'trending' }: HeroCardProps) {
  ...
  return (
    <div className="w-full max-w-full border border-rule bg-paper-2 rounded-[8px] p-4 sm:p-6 md:p-8 relative overflow-hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 300px' }}>
      <Link
        href={`/news/${post.id}/`}
      ...
      >
        <div className="flex items-start justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 min-w-0">
            ...
            <span className="w-px h-3 bg-rule flex-shrink-0" />
        ...
        {badge && (
          <span className="inline-block font-body text-[11px] sm:text-xs font-bold tracking-wider uppercase text-ink mb-3 sm:mb-4">
            {badge === 'breaking' ? 'Breaking Story' : 'Top Story'}
          </span>
        )}

        <h2 className="font-body font-[800] text-ink leading-[1.08] tracking-[-0.02em] mb-2 sm:mb-3 text-[clamp(1.3rem,4vw,2.2rem)]">
          {post.headline}
        </h2>

        <p className="font-body text-[13px] sm:text-[14px] text-ink-soft leading-[1.5] sm:leading-[1.6] line-clamp-3 max-w-3xl mb-3 sm:mb-4">
          {post.summary}
        </p>

        {firstHost && (
          <p className="font-body text-[11px] sm:text-[12px] text-ink-soft mb-3 sm:mb-4">
        ...
        <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-rule">
          <span className="font-body text-[10px] sm:text-[11px] text-ink-soft flex items-center gap-1.5">
            <ShieldIcon />
