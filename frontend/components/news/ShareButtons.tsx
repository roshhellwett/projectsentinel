...
export function ShareButtons({
  ...
  const copyLink = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        showToast(
          "Could not copy link. Try manually selecting the URL.",
          "error",
        );
      } finally {
  ...
  const handleShareClick = useCallback(async () => {
    ...
    if (
      ...
      navigator.share &&
      (isInline || isSheet || window.innerWidth < 768)
    ) {
      ...
      {showMenu && (
        <div
          ...
          className={cn(
            ...
            placement === "inline" &&
              "relative mt-3 w-full max-w-none shadow-paper-lift",
            placement === "sheet" &&
              "fixed bottom-[calc(6rem+env(safe-area-inset-bottom,0px))] left-3 right-3 w-auto max-w-none shadow-[0_16px_40px_-12px_rgba(0,0,0,0.24)] sm:absolute sm:bottom-full sm:left-auto sm:right-0 sm:mb-3 sm:w-[min(21rem,calc(100vw-2rem))]",
          )}
          style={{
            width:
              placement === "popover"
                ? "min(21rem,calc(100vw-2rem))"
                : undefined,
          }}
