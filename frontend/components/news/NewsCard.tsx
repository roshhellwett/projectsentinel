...
const NewsCardComponent = ({ post, onClick, isNew = false }: NewsCardProps) => {
  ...
  const accentColor = CATEGORY_BORDER_COLOR[post.category] ?? '#2563eb';
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        'hover:shadow-[0_16px_48px_-12px_rgba(0,0,0,0.65)]',
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          {sourceName ? (
            <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider truncate max-w-[110px]">
              {sourceName}
            </span>
          ) : (
          ...
          <span className="text-[10px] text-zinc-600 flex-shrink-0 ml-auto">
            {formatTimeAgo(post.published_at)}
          </span>
        </div>

        {/* Headline */}
        <h3 className="text-[16px] md:text-[17px] font-bold text-white/95 tracking-tight line-clamp-3 mb-2.5 leading-snug group-hover:text-white transition-colors duration-150">
          {post.headline}
        </h3>

        {/* Excerpt */}
        <p className="text-[12.5px] text-zinc-500 line-clamp-2 leading-relaxed mb-auto">
        ...
        <div className="flex items-center justify-between gap-2 mt-4 pt-3 border-t border-white/[0.05]">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              className="text-zinc-600 hover:text-[#25D366] transition-colors duration-200 p-0.5"
            ...
            <span className="text-zinc-600 group-hover:text-zinc-300 transition-colors duration-200">
