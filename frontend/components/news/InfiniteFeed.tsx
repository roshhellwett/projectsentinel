...
const NewsDrawer = dynamic(
  () => import("./NewsDrawer").then((m) => m.NewsDrawer),
  {
    ...
    loading: () => (
      <div
        className="fixed inset-0 bg-paper z-50 animate-fade-in"
        aria-hidden="true"
      />
    ),
  },
...
const FeedItem = memo(function FeedItem({
  ...
  const content = (
    <NewsCard post={post} onClick={handleClick} isNew={isNew} isRead={isRead} />
  );
  ...
  return (
    <div
      ...
      style={
        index < 6
          ? { animationDelay: `${Math.min(index, 4) * 0.03}s` }
          : undefined
      }
    >
      {content}
...
export function InfiniteFeed({
  ...
  const { posts, setPosts, loading, exhausted, freshIds, loadMore } =
    useInfiniteFeed({
      initialPosts,
      hasInitialMore,
      category,
  ...
  const readCount = useMemo(
    () => posts.filter((p) => readMap.has(p.id)).length,
    [posts, readMap],
  );
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 sm:py-24 px-4 text-center">
        <svg
          className="w-8 h-8 sm:w-12 sm:h-12 text-muted mb-2 sm:mb-4"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          aria-hidden="true"
        ...
        <h3 className="font-body font-bold text-sm sm:text-lg text-ink mb-1">
          {t("feed.no_news_title")}
        </h3>
    ...
    <>
      ...
      <NewsDrawer
        ...
        onSelectRelated={(next) => {
          ...
          recordRead();
          setPosts((prev) =>
            prev.some((p) => p.id === next.id) ? prev : dedupe([next, ...prev]),
          );
