...
export async function GET(request: Request) {
  ...
  const bypass =
    searchParams.get("_cb") !== null || searchParams.get("_poll") !== null; // cache-bust param for realtime/polling
    ...
    if (cached) {
      return NextResponse.json(cached, {
        headers: {
          ...CACHE_HEADERS,
          "X-Cache": "HIT",
          "Server-Timing": `cache;dur=${performance.now() - start}`,
        },
      });
    }
  }
  ...
  try {
    const { posts, nextCursor, hasMore } = await fetchPostsCursor(
      cursor,
      limit,
      category,
    );
