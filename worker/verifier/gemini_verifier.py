                ...
                try:
                    ...
                    if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                        ...
                        if attempt < max_retries - 1:
                            self.logger.log("GEMINI", f"Rate limited, waiting {retry_delay:.0f}s (attempt {attempt+1}/{max_retries})")
                            time.sleep(retry_delay)
                            continue
                    elif "503" in error_str or "UNAVAILABLE" in error_str:
                        wait = 15 * (attempt + 1)
                        if attempt < max_retries - 1:
                            self.logger.log("GEMINI", f"Service unavailable, waiting {wait}s (attempt {attempt+1}/{max_retries})")
                            time.sleep(wait)
                            continue
                    raise
